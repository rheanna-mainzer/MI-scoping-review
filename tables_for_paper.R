...
table1_data <- data[, c(
  "pub_year", "journal", "design_fct", "outcome_type",
...
table2_data <- data[, c(
  "inception_avail_fct", "analysis_reduc_fct",
...
table2 <- table2_data %>%
  gtsummary::tbl_summary(type = list(
    inception_avail_fct ~ "categorical",
...
table3_ft <- footnote(table3_ft,
                      i = 1, j = 2, value = as_paragraph("Values are n (%)"),
...
flextable(
  data[
    intersect(just_ID, grep("Yes", data$miss_justified)),
...
# Select and summarise data
table4_data <- data[, c(
  "prim_ana", "second_cond", "second_ana", "second_ana_just",
...
# Sub-table 2
table4_2_data <- filter(table4_data[, c(
  "second_cond", "second_ana", "second_ana_just",
  "mi_and_cca", "mi_cca_diff"
)], second_cond == "Yes")
...
# Select and summarise data
table5_data <- data[, c(
  "mi_m_reported", "mi_m", "mi_meth", "mi_soft",
